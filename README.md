...
- **FinanceKit:** Apple has granted the managed FinanceKit transaction-picker capability for `com.toto.Subwise`. The target declares `com.apple.developer.financekit.transaction-picker = true`, includes `NSFinancialDataUsageDescription`, and enables the guarded Wallet discovery path with `SUBWISE_FINANCEKIT_ENABLED = true`. On iOS 18+, Apple’s native picker lets the person explicitly select transactions; SubWise analyzes those selections locally and never requests unrestricted FinanceStore authorization. Earlier iOS versions retain Plaid, screenshot, and manual discovery.
